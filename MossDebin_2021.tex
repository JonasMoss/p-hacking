...
\[
\frac{\phi(x_{i}\mid\theta_{i},\sigma^2_{i})1_{[\alpha_{j-1},\alpha_{j})}(u_{i})}{\Phi(\sigma_{i}c_{j-1}\mid\theta,\sigma^2_{i})-\Phi(\sigma_{i}c_{j}\mid\theta,\sigma^2_{i})}=\phi_{[\sigma_{i}c_{j-1},\sigma_{i}c_{j})}(x_{i}\mid\theta,\sigma^2_{i}),
\]
...
The publication bias and the \textit{p}-hacking models have been defined as a selection model and a mixture model, respectively. To appreciate the difference between the model, consider the following example: we want to perform a meta-analysis based on 10 studies, for simplicity here with fixed effects (i.e., $\theta_i = \theta$, $\forall i$). In the ideal situation (no publication bias, no $p$-hacking), 10 researchers perform their studies fairly, observe an effect size from its distribution $\phi(x_{i}\mid\theta,\sigma^2_{i})$ and publish the result. To perform our meta-analysis, we do not need much more that estimate $\theta$ by using $\phi(x_{i}\mid\theta,\sigma^2_{i})$.

In a publication bias scenario, instead, some of those 10 studies are not published because their p-values $u_i$. Let us say that the first 3 are smaller than $0.025$ (group A), the following 4 are between $0.025$ and $0.05$ (group B), and the remaining 3 are larger than $0.05$ (group C). With an editor publishing all studies with $u_i < 0.025$, half of those with $0.025 \leq u_i < 0.05$ and one-third when $u_i \geq 0.05$ (i.e., $w(u_i) = 1_{[0,0.025)}(u_i) + 0.5 \cdot 1_{[0.25,0.05)}(u_i) + 0.33 \cdot 1_{[0.05, 1]}(u_i)$), we would expect to see all 3 studies in group A, 2 of Group B and only one of group $C$. It is clear that the density is not anymore the original $\phi(x_{i}\mid\theta,\sigma^2_{i})$, but a transformed one in which some values of $\theta$ are under-represented by the effect of a selection mechanism.

In the \textit{p}-hacking scenario, instead, we observe all our 10 studies, but not with their original effect size, because they have been modified in order to reach a specific significance value. 
...
We want to answer these three questions about the \textit{p}-hacking and publication bias models: (1) Do they work even in the absence of \textit{p}-hacking and publication bias? Although we know these phenomena are ubiquitous and should always be corrected for, it is still important that the models do not distort the results when there is no publication bias or \textit{p}-hacking. (2) How do they behave in extreme situations, in particular when $n$ is small and the heterogeneity is large? (3) %Are the models distinguishable in practice?
...
%$\textsc{elpd}$ is the expected log pointwise predictive density for a new data set and  is an estimate of this quantity by leave-one-out cross validation.
...
For simplicity of exposition, in this paper we only considered normal densities, but the theory holds more generally. A remaining concern is identifiability, but we show in Web Appendix D that the publication bias and the \textit{p}-hacking models are identifiable under weak conditions on $f$.
