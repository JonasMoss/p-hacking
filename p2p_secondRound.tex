...
We added the proof of the fixed effect model and referred to a new Web Appendix (Web Appendix B) for the similar proof of the random effect model.
...
In this version that integral is removed. It is not important to describe our model, so we preferred to completely remove it to avoid confusion.
...
True, thanks a lot for pointing that out! We edited the formulas. In order to keep the exposition clear, we added $\sigma_i$ to $c_j$ and use the latter extensively in this and other formulas.
...
As suggested, we display the formula of the truncated normal. The newly added proof of Proposition 1 should also help relating $u_i$ to $w(u_i)$ and $x_i$, as also the new Web Appendices A, B and D. As mentioned in the answer to the first concern, we reformulate $\omega$ by using $1(\alpha = \alpha_j)$ instead of $1_{(0, \alpha_{j})}$ to clarify that the p-hacking is done to a specific value of $\alpha$, generated from $\omega$. We also removed redundant dependencies to make the formulas easier to read.
...
We clarified that $u_i$ is the p-value associated to the study in which $x_i$ is observed. We also added the subscript $i$ to $u$ to make the relationship more clear.
