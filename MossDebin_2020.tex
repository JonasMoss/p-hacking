...
\end{equation}
where $H^c$ is the complement of $H$. This model can be be viewed as a rejection sampling model \citep{von1951various}.

\begin{prop}\label{prop:is density}
The function $f_{H}(y)$ is a density for any $H$.
\end{prop}
See the Web Appendix A for a proof of Proposition \ref{prop:is density}. The choice of $H$ strongly affects the form of $f_{H}(y)$, and, in general, $f_{H}(y) \neq f_{G}(y)$ for $H \neq G$.
...
\end{equation}
by the selection probability $w(u_i) \in [0,1]$, which is a probability for each $u_i$. Here $u_i$ is a \textit{p}-value that depends on $x_{i}$ and maybe something else, such as the standard deviation of $x_{i}$, but does not depend on $\theta_{i}$. In the notation of model \eqref{eq:H-selection model}, $y_H = \{x_i, \eta_i, \theta_i\}$, so that $w(u_i) = w(u_i(x_i, \eta_i \mid \theta_i)) = p(s=1 \mid x_i, \theta_i, \eta_i)$. Note that $w(u_i)$ cannot depend on $\theta_{i}$ since the editor has no way of knowing the parameter $\theta_{i}$; if she did, she would not have to look at the \textit{p}-values at all. Nevertheless, $\theta_i$ is in the selection set, as the selection happens for each study given the $\theta_i$. This point will be more clear in Section \ref{subsec:Selection sets, meta analysis}. The selection probability $w(u_i)$ might depend on other quantities modelled by $\eta_{i}$ though, if $\eta_{i}$ is known to the editor. The normalizing constant of model \eqref{eq:Publication bias model} is finite for any probability $w(u_i)$, hence $f$ is a \textit{bona fide} density.
...
The difference between the models is a direct consequence of Proposition \ref{prop:Equal selection models}, as the selection sets are different. The idea behind the publication bias model is that a completely new study is done whenever the last one failed to be published. This implies that each event of publication bias depends on the $\theta_i$ and $x_i$ of the specific study. Moreover, since the editor's decision to publish is also study-specific, the selection set also contains a variable $Z$, living on the unit interval, that encodes her choice: If the observed \textit{p}-value is less than $Z$, the study is published. Since $x_i$ and $\theta_i$ are sampled together, the selection mechanism modifies $p(\theta_i)$.

In the \textit{p}-hacking scenario, the \textit{p}-hacker will hack his study all the way to significance, regardless of $\theta_i$. This means that $\theta_i$ must be placed outside the selection set. Moreover, the decision of how much to \textit{p}-hack is not reevaluated at each attempt. Consequently, the random variable that controls the \textit{p}-hacking decisions, analogously to the publication bias model, $Z$, is also placed outside the selection set. This is the case, for example, of an author who decides to \textit{p}-hack to level $\alpha$ ($Z = \alpha$): he acts on $x_i$ to obtain the desired \textit{p}-value, whatever the sampled $\theta_i$ is. Since $\theta_i$ does not belong to the selection set, the selection mechanism does not modify $p(\theta_i)$.

The Web Appendix C contains an example that clarifies the different effect of the selection mechanism on $p(\theta_i)$ and a graphical representation of the two models, while the Web Appendix B a further proof of the model differences. The publication bias model defined in Proposition \ref{prop:One-sided normal discrete probability vector publication bias model-1} and the \textit{p}-hacking model are equivalent when $\sigma_{i}$ is fixed across studies. This holds both for the fixed and random effects models. To see this, let $\pi$ be any probability vector for the \textit{p}-hacking model and solve the invertible linear system $\pi^{\star}(\rho)=\pi$ for $\rho$. There is no guarantee for the models to be equivalent when $\sigma_{i}$ is not fixed. Finally, Web Appendix D addresses the issue of identifiability, and shows that the two models are identifiable under weak conditions on $f$.
...
Further modification to the models can be obtained by modifying their selection sets. For example, in the publication bias model, the nuisance parameter $\eta_i$ (which can include, e.g., the standard deviation $\sigma_i$) could be incorporated. Although the common practice in meta-analysis studies is to treat the standard deviations as nuisance parameter \citep[see, e.g.,][]{vanHouwelingen2002}, the actual tests usually contain an estimate of the standard error. If the $\eta_i$s are in the selection set, they can influence the study's acceptance. A possible modification of the \textit{p}-hacking model consists in putting $\theta_i$ inside the selection set, which makes the researcher draw new $\theta_i$s every time he attempts a \textit{p}-hack. This could be used to model scenarios where the hypothesis is not known in advance by the researchers.
